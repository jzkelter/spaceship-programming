...
md"# Spaceship Programming Data"

# ╔═╡ d1123afe-395d-4712-a456-3d4c7a46757d
md"""
[page 1 spaceship programming NetTango model](https://ct-stem.s3.amazonaws.com/uploads/jzkelter/2022/01/21/spaceship%20programming%20no%20maze_2022-01-21-11-18-09-529113.html)
...
[group google doc](https://docs.google.com/document/d/1etfsnP9UWp9a_wHwFTil2uR4tteOgADXbyjiDjYuN_Y/edit#heading=h.6tej0macxh72)
...
md"""
## Final distance and speed vs attempt graphs
The "Final distance from target vs attempt" plot shows the distance of the ship when the model stopped. This means that if the ship flew off the screen, it still shows a finite distance. This is perceptually what the student would have experienced as how far the ship was even though in "reality" the ship would have kept getting further away if the ship had non-zero speed.  
"""

# ╔═╡ c6552640-3241-4da4-8b1f-a7439896635c
md"""
## Individual trajectory visualizations
In the following graphs, a dot is drawn at each tick of the model. A blue dot represents the ship moving without its engines on (drifting) and an orange dot represents the ship moving with its engines one (accelerating). Sometimes the dots are too close together to tell them apart, which means the ship was moving slowly.
"""

# ╔═╡ 2b59b381-1539-48bd-bec0-a1e786da21a0
md"""
...
function convert_to_time_since_start(
	datetime, 
	start=DateTime(2023, 02, 28, 20, 15, 51)
)
	difference_in_milliseconds = datetime - start
	total_seconds = Int(Dates.value(difference_in_milliseconds) / 1000)
...
function plot_final_dist_and_speed_vs_attempt(sdf)
	x_lim = length(sdf.final_speed)
	f2 = Figure(size=(500, 600))	
	axis = (; title="Final distance from target vs attempt", 
			  limits=(0, x_lim, 0, nothing))
	fig1 = plot(f2[1, 1], sdf.final_distance; axis=axis)	
	axis2 = (; title="Final speed vs attempt",
				limits=(0, x_lim, nothing, nothing))
	fig2 = lines(f2[2, 1], sdf.final_speed; axis=axis2)
	f2
end

# ╔═╡ 95220c54-1b62-49c2-a9d9-f953866947d2
function datetime_to_video_time(tstamp)
	third_tstamp = DateTime("2022-01-21T20:25:04")  # This time was 2:51
	third_vid_time = Millisecond(2 * 60_000 + 51_000)  # 2:51
	vid_time = tstamp - third_tstamp + third_vid_time
	return Dates.format(Time(0) + Millisecond(vid_time), "M:SS")
end

# ╔═╡ 0f4f678c-28fa-40d1-8d59-41829a6ecd52
function render_blocks(sdf_row)
	sched = JSON.parse(sdf_row.engineSchedule)
	text = ""
	for block in sched
		duration = block[1][2] - block[1][1]
		engines = block[2]
		if all(engines .== 0) 
...
md"# Functions to Process Data"

# ╔═╡ 4741a9bd-a4f3-4dad-9ae4-bb3d8d3a25d0
# path_to_engines_data = "../data/2022/spaceship_page1_data_lsts.csv" # 2022
...
function read_spaceship_data_to_df()	
	datetimeify(date_str) = DateTime(date_str[1:19], dateformat"yyyy-mm-dd HH:MM:SS.sss")
	path = path_to_engines_data
	df =  CSV.read(path, DataFrame)
	transform!(df, :timestamp => (ts -> datetimeify.(ts)) => :timestamp)

	just_engines = attempt -> map(row -> JSON.json(row[2]), JSON.parse(attempt))
	df[!, "enginesWOTiming"] = map(just_engines, df.engineSchedule)
	
	sort!(df, [:userId, :timestamp])
	return select!(df, Not([:Column1, :index]))
end
...
# ╔═╡ f15f6296-1b68-425b-9375-379cd3709e1e
function histogram_of_blocks_and_timing_repetitions()	
	counts = combine(groupby(sdf, :engineSchedule), nrow => :count)
	n_bins = max(counts.count...) - 1
	axis = (; 
		title="Unique B&T repeat distribution",
		xlabel="Repetitions",
		ylabel="# Unique Block and Timing Combos",
		xticks=(0:1:n_bins)
	)
	hist(counts.count; bins=n_bins, axis=axis, strokewidth=1)	
end

# ╔═╡ 39d5541f-74c3-40ca-97d9-51da2a40c5f0
histogram_of_blocks_and_timing_repetitions()

# ╔═╡ 57c54131-d6e0-4a2c-81a8-52f959864dec
histogram_of_blocks_and_timing_repetitions()

# ╔═╡ 012772ce-15e7-47f7-a5ff-a3c8c6da7885
function histogram_of_blocks_repetitions()		
	
	counts = combine(groupby(sdf, :enginesWOTiming), nrow => :count)

	n_bins = max(counts.count...) - 1
	axis = (; 
		title="Unique Block Repetitions Distribution",				
		xlabel="repetitions",
		ylabel="# block combos",
		xticks=(0:1:n_bins)		
	)
...
# ╔═╡ 03abc127-4149-4d5b-8fb3-3802cecb1a85
histogram_of_blocks_repetitions()

# ╔═╡ 1aa0537f-5c47-49c4-a027-fbbe864db0b3

function plot_trajectory_no_maze(sdf_row, attempt_num)
	traj = JSON.parse(replace(sdf_row.trajectory, "'" => "\""))
	xs = [convert(Float32, x) for x in traj["xcor"]]
	ys = [convert(Float32, y) for y in traj["ycor"]]

	if userId in [26807, 26815]  ## one of the two students in the videos
		time = datetime_to_video_time(sdf_row.timestamp)
	else
		
		# time = Dates.format(sdf_row.timestamp, "HH:MM:SS")
		time = convert_to_time_since_start(sdf_row.timestamp)
	end
		
	axis = (; title=string("student ", sdf_row.userId, " attempt ", attempt_num, " at ", time))
	...
	lines!([6.5, 8.5, 8.5, 6.5, 6.5], [6.5, 6.5, 8.5, 8.5, 6.5], color="green") 
	limits!(-16, 16, -16, 16)			
	return fig
end

# ╔═╡ a2191369-6f65-4b6b-966b-61e722b31b98
function render_trajectories_and_blocks(sdf)			
	return PlutoUI.combine() do Child
		...
		inputs = [
			...
			| Blocks      | Trajectory |
			| :----------- | ----------- |
			| $(Markdown.parse(render_blocks(sdf[i, :]))) | $(plot_trajectory_no_maze(sdf[i, :], i))	|						
			"""
...
function get_student_rdf(trdf, userId)
	filter(:page_num => ==(1), filter(:userId => ==(userId), trdf))
end


# ╔═╡ 39b7cefe-a973-4cc7-956d-11f74aec3b72
srdf = get_student_rdf(rdf, userId);
...
function quantify_attempts_by_type(sdf)
	total_attempts = length(sdf.engineSchedule)
	unique_block_and_timings = length(Set(sdf.engineSchedule))	

	# Strip out timing and just leave engines for an attempt
	just_engines = attempt -> map(row -> row[2], JSON.parse(attempt))

	unique_blocks = length(Set(map(just_engines, sdf.engineSchedule)))
...
md"""
### Old code 
when I was showing all trajectories in a single plot with many sub-plots
"""


# ╔═╡ 396f5344-7790-4207-92b8-7c859cb907be
function plot_trajectory_no_maze2(sdf_row, attempt_num, f)
	# This is the old version I used before I switched to rendering a table with blocks
	traj = JSON.parse(replace(sdf_row.trajectory, "'" => "\""))
	xs = [convert(Float32, x) for x in traj["xcor"]]
	ys = [convert(Float32, y) for y in traj["ycor"]]
	axis = (; title=string("student ", sdf_row.userId, " attempt ", attempt_num, " at ", Dates.format(sdf_row.timestamp, "HH:MM:SS")))

	i = Int(ceil(attempt_num / 2))
	j = (attempt_num + 1) % 2			
	fig = scatter(f[i, j], xs, ys; 
						 color=traj["color"], 							
						 markersize=4,
						 axis=axis
	)		
...
function plot_all_trajectories(sdf)
	l = nrow(sdf)
	res = 330
	f = Figure(resolution = (2 * res, res * (l / 2)))		
	for i in (1:l)
		plot_trajectory_no_maze2(sdf[i, :], i, f)		
	end	
	f
end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "47567f24178e8871ef58dc9e515e42d8182be808"
